...
class Notificacion(models.Model):
    """Modelo para notificaciones en tiempo real del sistema"""
    TIPO_CHOICES = [
        ('nueva_ficha', 'Nueva Ficha de Emergencia'),
        ('solicitud_medicamento', 'Solicitud de Medicamento'),
        ('solicitud_examen', 'Solicitud de Examen'),
        ('cambio_estado', 'Cambio de Estado de Ficha'),
        ('signos_vitales', 'Nuevos Signos Vitales'),
    ...
    usuario_destinatario = models.ForeignKey(Usuario, on_delete=models.CASCADE, related_name='notificaciones')
    ficha = models.ForeignKey('FichaEmergencia', on_delete=models.CASCADE, null=True, blank=True, related_name='notificaciones')
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['usuario_destinatario', '-fecha_creacion']),
            models.Index(fields=['usuario_destinatario', 'leida']),
        ]
    
    def __str__(self):
        return f"{self.titulo} - {self.usuario_destinatario.username} ({'Leída' if self.leida else 'No leída'})"
