...
export function ModalDiagnostico({ open, onOpenChange, ficha, onConfirm }: ModalDiagnosticoProps) {
  ...
  const handleDescargarFicha = async () => {
    if (!ficha) return
    try {
      await documentosAPI.descargarFichaPDF(Number(ficha.id))
      toast({
  ...
  const handleDescargarReceta = async () => {
    if (!ficha) return
    try {
      await documentosAPI.descargarRecetaPDF(Number(ficha.id))
      toast({
        title: "PDF descargado",
  ...
  const handleDescargarOrdenExamenes = async () => {
    if (!ficha) return
    try {
      await documentosAPI.descargarOrdenExamenesPDF(Number(ficha.id))
      toast({
        title: "PDF descargado",
  ...
  const handleDescargarAlta = async () => {
    if (!ficha) return
    try {
      await documentosAPI.descargarAltaPDF(Number(ficha.id))
      toast({
        title: "PDF descargado",
  ...
  const handleConfirm = () => {
    ...
    if (!formData.codigoCIE10 || !formData.diagnostico || !formData.indicaciones || !formData.tipoAlta) {
      setError('Por favor complete todos los campos obligatorios: Código CIE-10, Diagnóstico, Indicaciones y Tipo de Alta')
      return
    }
        ...
        <div className="space-y-4 py-4">
          ...
          {ficha.anamnesis?.alergiasCriticas && (
            <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
              <p className="text-sm font-semibold text-red-500">ALERGIAS MEDICAMENTOSAS:</p>
