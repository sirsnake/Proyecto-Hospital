...
export function ModalExamenes({ open, onOpenChange, ficha, onConfirm }: ModalExamenesProps) {
  ...
  const handleConfirm = () => {
    ...
    if (!formData.tipoExamen || !formData.examenesEspecificos || !formData.justificacion) {
      setError('Por favor complete todos los campos obligatorios: Tipo de Examen, Exámenes Específicos y Justificación Clínica')
      return
    }
  ...
  const signosVitales = Array.isArray(ficha.signosVitales) 
        ...
        <div className="space-y-4 py-4">
            ...
            {signosVitales ? (
              <div className="grid grid-cols-4 gap-2 text-sm">
                ...
                <div>
                  ...
                  <span className="text-white">{signosVitales.frecuenciaCardiaca || '-'} lpm</span>
                </div>
                <div>
                  <span className="text-slate-400">SatO₂:</span>{" "}
                  <span className="text-white">{signosVitales.saturacionO2 || '-'}%</span>
                </div>
                <div>
                  <span className="text-slate-400">Temp:</span>{" "}
                  <span className="text-white">{signosVitales.temperatura || '-'}°C</span>
                </div>
              </div>
            ) : (
              <p className="text-sm text-amber-400">Sin signos vitales registrados</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.tipoExamen} onValueChange={(value) => setFormData({ ...formData, tipoExamen: value })}>
              ...
              <SelectContent>
                <SelectItem value="laboratorio_sangre">Laboratorio de Sangre</SelectItem>
                <SelectItem value="laboratorio_orina">Examen de Orina / Urocultivo</SelectItem>
                <SelectItem value="imagenes_rx">Radiografía (Rx)</SelectItem>
                <SelectItem value="imagenes_tac">Tomografía (TAC)</SelectItem>
                <SelectItem value="imagenes_rmn">Resonancia Magnética (RMN)</SelectItem>
                <SelectItem value="imagenes_eco">Ecografía</SelectItem>
                <SelectItem value="ecg">Electrocardiograma</SelectItem>
                <SelectItem value="gases">Gases Arteriales</SelectItem>
                <SelectItem value="microbiologia">Microbiología / Cultivos</SelectItem>
...
• Sangre: Hemograma completo, Perfil bioquímico, Perfil lipídico, PCR, Electrolitos plasmáticos, Función renal, Función hepática, Coagulación
• Orina: Orina completa, Urocultivo con antibiograma
• Imágenes: Rx tórax AP/L, TAC abdomen con contraste, etc."
